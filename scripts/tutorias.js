...
router.patch('/solicitudes/:id/aceptar', auth('profesor'), async (req, res) => {
  ...
  if (!precio_cop) {
    return res.status(422).json({ error: 'Debes indicar el precio de la sesión' });
  }
  ...
  try {
    ...
    const [tRes] = await conn.query(
      ...
      [
        ...
        precio_cop, link_virtual || null, direccion || null,
      ]
    );
...
//  El estudiante cancela su propia solicitud (solo si está 'enviada').
// ─────────────────────────────────────────────────────────────
router.delete('/solicitudes/:id', auth('estudiante'), async (req, res) => {
  const solicitudId = parseInt(req.params.id);

  try {
    const [sol] = await db.query(
      "SELECT * FROM solicitudes WHERE id = ? AND estudiante_id = ?",
      [solicitudId, req.usuarioId]
    );
    ...
    if (sol[0].estado !== 'enviada' && sol[0].estado !== 'pendiente') {
      return res.status(409).json({ error: 'Solo puedes cancelar solicitudes que aún no han sido respondidas' });
    }

    await db.query("DELETE FROM solicitudes WHERE id = ?", [solicitudId]);
