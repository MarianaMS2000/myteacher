...
document.addEventListener('DOMContentLoaded', function () {
  ...
  cargarPreguntasGuardadas(); /* Cargar preguntas guardadas en localStorage */
});
...
function publicarPregunta() {
  ...
  var stored = JSON.parse(localStorage.getItem('mt_user') || '{}');
  guardarPreguntaEnStorage({
    id:       nuevoId,
    texto:    texto,
    autor:    nombreUsuario,
    iniciales: iniciales,
    tags:     tagActuales.slice(),
    tiempo:   new Date().toLocaleDateString('es-CO', { day:'numeric', month:'short', year:'numeric' })
  });
...
var MT_COMUNIDAD_KEY = 'mt_comunidad_preguntas';

/** Guarda la pregunta actual en localStorage */
function guardarPreguntaEnStorage(pregunta) {
  try {
    var stored = JSON.parse(localStorage.getItem(MT_COMUNIDAD_KEY) || '[]');
    /* Limitar a 50 preguntas para no llenar el storage */
    stored.unshift(pregunta);
    if (stored.length > 50) stored = stored.slice(0, 50);
    localStorage.setItem(MT_COMUNIDAD_KEY, JSON.stringify(stored));
  } catch(e) { console.warn('No se pudo guardar pregunta en localStorage:', e); }
}

/** Carga las preguntas guardadas y las inserta antes de las de ejemplo */
function cargarPreguntasGuardadas() {
  try {
    ...
    /* Insertar en orden (más reciente primero) */
    stored.forEach(function(p) {
      /* Verificar que no esté ya en el DOM (por si acaso) */
      if (document.getElementById(p.id)) return;
      var card = crearCardDesdeDatos(p);
      /* Insertar al inicio, antes de las preguntas de ejemplo */
      lista.insertBefore(card, lista.firstChild);
    });

    actualizarContadorPreguntas();
  } catch(e) { console.warn('Error cargando preguntas guardadas:', e); }
}

/** Crea un elemento DOM de card a partir de datos serializados */
function crearCardDesdeDatos(p) {
  var card = document.createElement('div');
  ...
  card.innerHTML =
    '<div class="question-card-header">' +
      '<div class="question-user-avatar" style="background:linear-gradient(135deg,#1e4620,#4a7a30);">' +
        escapeHtml(p.iniciales || '?') +
      '</div>' +
      '<div class="question-user-info">' +
        '<div class="question-user-name">' + escapeHtml(p.autor) + '</div>' +
        '<div class="question-time"><i class="fa-regular fa-clock"></i> ' + escapeHtml(p.tiempo) + '</div>' +
      '</div>' +
    '</div>' +
    '<div class="question-text">' + escapeHtml(p.texto) + '</div>' +
    (tagsHTML ? '<div class="question-tags">' + tagsHTML + '</div>' : '') +
    '<div class="question-card-footer">' +
      '<button class="btn-like" onclick="toggleLike(this)">' +
        '<i class="fa-regular fa-heart"></i> <span class="like-count">0</span>' +
      '</button>' +
      '<button class="btn-see-answers" onclick="verRespuestas(\'' + p.id + '\', this, \'' +
        escapeJs(p.texto) + '\')">' +
        '<i class="fa-regular fa-comment-dots"></i> Ver respuestas (IA)' +
      '</button>' +
    '</div>' +
    '<div class="answers-section" id="answers-' + p.id + '"></div>';
