...
router.post('/registro', async (req, res) => {
  ...
  try {
    // PASO 2: Verificar email único — tanto cuentas activas como eliminadas.
    // Si el email ya existe (activo o eliminado), no permitimos nuevo registro.
    const [existe] = await db.query(
      'SELECT id, activo FROM usuarios WHERE email = ? LIMIT 1',
      [emailNorm]
    );
    if (existe.length > 0) {
      if (existe[0].activo === 1) {
        return res.status(409).json({ error: 'Ya existe una cuenta con ese correo electrónico.' });
      } else {
        // Cuenta eliminada — bloqueamos el email permanentemente
        return res.status(409).json({ error: 'Este correo electrónico no puede usarse para registrarse.' });
      }
    }
    const idReutilizado = null; // nunca reutilizamos cuentas eliminadas
    ...
    try {
      ...
      // Siempre insertar como cuenta nueva
